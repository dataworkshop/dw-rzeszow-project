...
def proc_unsharp_mask(input_img):
    sharpened_image = unsharp_mask(input_img)
...
def show_images(input_img):
    ...
    while(True):
        ...
        cv2.imshow("Final result", frame_cuttedThresh)
...
def main():
    ...
    input_spec = sys.argv[2]

    if select_input == 'cam':
        print("Take a picture with webcam, ESC to take picture...")
        capture = cv2.VideoCapture(int(input_spec))  #czasami zmiana na wartość -1, 0, 1, włącza kamerę
        ...
        cv2.destroyAllWindows()
        input_img = cv2.imread('frame.jpg') 
       
    if select_input == 'file':
        input_img = cv2.imread(input_spec) 
    
    if select_input != 'cam' and select_input != 'file':
        print("======== Please read read.me ! ============")
        print("===========================================")
      
    proc_unsharp_mask(input_img)
    proc_bitwise_not()
    captureAndCrop()
